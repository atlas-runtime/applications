...
void
ecall_get_server_key(uint8_t *key, int32_t k)
{
    memcpy(key, server_public_key, k);
}

void
ecall_register_client_key(uint8_t *key, int32_t k)
{
...
void
ecall_generate_shared_key(int32_t fd)
{
    Content c;
...
void
ecall_send_pubkey(int fd)
{
    ocall_send_packet(fd, server_public_key, (int32_t)crypto_box_PUBLICKEYBYTES);
}
...
void
ecall_start(int32_t socket_fd, uint8_t *buf, uint32_t len)
{
    uint8_t *og;
